...
function Web123Page(props) {
   ...
   const handleOkEditModal = async () => {
      ...
      try {
         const response = await fetch(
            `/api/tracking/web123?id=${selectedRow.id}`,
            {
               method: "PATCH",
               headers: {
...
Web123Page.getLayout = function getLayout(page) {
   return <Layout>{page}</Layout>
}
export async function getServerSideProps(context) {
   const session = await getSession({ req: context.req })
   // get trackings shimizu
   const api_tracking_web123 = `${process.env.BACKEND_URL}/api/tracking/web123`
