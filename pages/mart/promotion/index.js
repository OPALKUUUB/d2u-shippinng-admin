...
function MartPromotionPage(props) {
   ...
   const handleOkEditModal = async () => {
      ...
      try {
         const response = await fetch(
            `/api/mart/promotion?id=${selectedRow.id}`,
            {
               method: "PATCH",
               headers: {
   ...
   const handleOkAddModal = async () => {
      ...
      try {
         const response = await fetch("/api/mart/promotion", {
            method: "POST",
            headers: {
               "Content-Type": "application/json",
            },
   ...
   return (
      <Fragment>
         <CardHead name="Mart Promotion Page" />
...
MartPromotionPage.getLayout = function getLayout(page) {
   return <Layout>{page}</Layout>
}

export async function getServerSideProps(context) {
   const session = await getSession({ req: context.req })
   const api = `${process.env.BACKEND_URL}/api/mart/promotion`
...
export default MartPromotionPage
