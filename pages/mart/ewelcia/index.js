...
function MartEwelciaPage(props) {
   ...
   const handleOkEditModal = async () => {
      ...
      try {
         const response = await fetch(
            `/api/mart/ewelcia?id=${selectedRow.id}`,
            {
               method: "PATCH",
               headers: {
   ...
   const handleOkAddModal = async () => {
      ...
      try {
         const response = await fetch("/api/mart/ewelcia", {
            method: "POST",
            headers: {
               "Content-Type": "application/json",
            },
   ...
   return (
      <Fragment>
         <CardHead name="Mart Gachapong Page" />
...
MartEwelciaPage.getLayout = function getLayout(page) {
...
export default MartEwelciaPage
